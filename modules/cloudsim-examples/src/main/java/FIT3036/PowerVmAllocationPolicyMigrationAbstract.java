...
public abstract class PowerVmAllocationPolicyMigrationAbstract extends PowerVmAllocationPolicyAbstract {
    ...
    protected boolean isHostOverUtilizedAfterAllocation(PowerHost host, List<Vm> vmsToAllocate) {
        boolean isHostOverUtilizedAfterAllocation = true;

        ArrayList<Vm> allocatedVm = new ArrayList<Vm>();

        for (Vm vm : vmsToAllocate)
            if (host.vmCreate(vm))
                allocatedVm.add(vm);

        if (allocatedVm.size() == vmsToAllocate.size()) {
            isHostOverUtilizedAfterAllocation = isHostOverUtilized(host);
        } else {
            isHostOverUtilizedAfterAllocation = false;
        }

        for (Vm vm : allocatedVm)
            host.vmDestroy(vm);
    ...
    protected double getPowerAfterAllocation(PowerHost host, List<Vm> vmsToAllocate) {
        double power = 0;
        try {
            power = host.getPowerModel().getPower(getMaxUtilizationAfterAllocation(host, vmsToAllocate));
        } catch (Exception e) {
            e.printStackTrace();
            System.exit(0);
        }
    ...
    protected double getMaxUtilizationAfterAllocation(PowerHost host, List<Vm> vmsToAllocate) {
        double requestedTotalMips = 0;
        for (Vm vm : vmsToAllocate)
            requestedTotalMips += vm.getCurrentRequestedTotalMips();
        double hostUtilizationMips = getUtilizationOfCpuMips(host);
        double hostPotentialUtilizationMips = hostUtilizationMips + requestedTotalMips;
        double pePotentialUtilization = hostPotentialUtilizationMips / host.getTotalMips();
        return pePotentialUtilization;
